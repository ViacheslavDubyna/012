...
"""
Модуль для генерації тестових даних для інформаційно-аналітичної системи ДПСУ
...
# Контрольні точки перетину кордону
CHECKPOINTS = [
    'Краковець', 'Шегині', 'Рава-Руська', 'Ягодин', 'Устилуг',
    'Порубне', 'Дякове', 'Ужгород', 'Чоп', 'Малий Березний',
    'Могилів-Подільський', 'Платонове', 'Мамалига', 'Кучурган'
]
...
# Типи інцидентів
INCIDENT_TYPES = [
    'Незаконний перетин кордону', 'Контрабанда товарів', 'Контрабанда наркотиків',
    'Підробка документів', 'Порушення правил перебування', 'Спроба вивезення зброї',
    'Порушення правил перетину', 'Незаконне перевезення людей', 'Порушення митних правил'
]
...
def generate_incidents(count=100):
    """
Генерація тестових інцидентів на кордоні
...
def generate_border_crossings(count=500):
    """
Генерація тестових даних про перетин кордону
...
:return: список об'єктів BorderCrossing
    ...
    for _ in range(count):
        # Вибір випадкового пункту пропуску та напрямку
        ...
        # Кількість перетинів (більше для легкових авто, менше для вантажних)
        if vehicle_type == 'легковий':
            count_value = random.randint(50, 500)
        elif vehicle_type == 'вантажний':
        ...
        # Створення запису про перетин кордону
...
:return: кортеж (X, y) з ознаками та цільовими значеннями
    ...
    data = {
        ...
        'кількість_перетинів': np.random.normal(1000, 300, count),
    ...
    for i in range(count):
        # Більше інцидентів у вихідні, влітку, під час свят і при більшій кількості перетинів
        risk_score = 0
...
def generate_training_data_for_border_crossing_predictor(count=1000):
    """
Генерація тренувальних даних для моделі прогнозування перетинів кордону
...
:return: кортеж (X, y) з ознаками та цільовими значеннями
    ...
    for i in range(count):
        ...
        # Більше перетинів у вихідні
        if data['вихідний'][i] == 1:
            base_count += 300
        
        # Більше перетинів влітку (6-8 це червень-серпень)
        if 6 <= data['місяць'][i] <= 8:
            base_count += 200
        
        # Більше перетинів у свята
        if data['свято'][i] == 1:
            base_count += 400
        
        # Різні пункти пропуску мають різну завантаженість
...
:return: кортеж (X, y) з ознаками та цільовими значеннями
    ...
    data = {
        'день_тижня': np.random.randint(0, 7, count),  # 0-6 (пн-нд)
        'місяць': np.random.randint(1, 13, count),  # 1-12
        'прогноз_перетинів': np.random.normal(1000, 300, count),
        'рівень_загрози': np.random.choice(['низький', 'середній', 'високий'], count, p=[0.7, 0.2, 0.1]),
        'довжина_кордону_км': np.random.normal(50, 10, count),
        'кількість_пунктів_пропуску': np.random.randint(1, 5, count)
    }
    ...
    for i in range(count):
        ...
        # Додаємо персонал в залежності від кількості перетинів
        ...
        # Додаємо персонал в залежності від довжини кордону
        base_personnel += data['довжина_кордону_км'][i] / 5
        
        # Додаємо персонал в залежності від кількості пунктів пропуску
...
Заповнення бази даних тестовими даними
    ...
    # Генерація даних про перетин кордону
